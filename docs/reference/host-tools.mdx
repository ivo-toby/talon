...
Talon exposes 9 built-in tools to agents through the internal MCP server `__talond_host_tools`.
...
Internal tool: `persona.send`

Capability labels:

- `persona.send:*`

Sends a task to another persona via the A2A protocol and optionally waits for the reply.

| Parameter | Type | Notes |
|---|---|---|
| `target_persona` | string | Required; name of the destination persona |
| `message` | string | Required; the task or message to send |
| `await_reply` | boolean | Optional; if `true`, blocks until the target persona responds |

Notes:

- The task is delivered through the durable queue, so the target persona processes it asynchronously by default.
- Set `await_reply: true` to wait for the response inline. Use with care on slow or long-running personas — it will block the calling agent for the full duration.
- The calling persona must have the `persona.send:*` capability grant.

---

## `persona_list`

Internal tool: `persona.list`

Capability labels:

- `persona.send:*`

Lists all loaded personas available for A2A task delegation.

Returns an array of persona cards:
...
|---|---|---|
| `name` | string | Persona name, used as `target_persona` in `persona_send` |
| `description` | string \| null | First line of the persona's system prompt file, `#` stripped |
| `skills` | string[] | Skills attached to the persona |

Notes:

- The calling persona is excluded from the results.
- Call this before `persona_send` if you need to discover which personas are available and what they can do.
- Requires the same `persona.send:*` capability as `persona_send`.
