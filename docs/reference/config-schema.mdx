...
Runtime guardrails: spending caps, rate limiting, and loop detection. All sub-sections are optional. Omit the entire block to disable governance.
...
| Field | Type | Default | Description |
|---|---|---|---|
| `daily_token_cap` | integer > 0 | — | Max tokens per persona per 24h rolling window |
| `hourly_token_cap` | integer > 0 | — | Max tokens per persona per 1h rolling window |
| `warn_at_percent` | number 1–100 | `80` | Log a warning when usage exceeds this percentage |
| `per_persona` | record | — | Per-persona cap overrides (keys are persona names) |
| `per_persona.<name>.daily_token_cap` | integer > 0 | — | Persona-specific daily cap |
| `per_persona.<name>.hourly_token_cap` | integer > 0 | — | Persona-specific hourly cap |

### `governance.rate_limits`

| Field | Type | Default | Description |
|---|---|---|---|
| `inbound_per_minute` | integer > 0 | `30` | Max inbound messages per channel per minute |
| `inbound_per_user_per_minute` | integer > 0 | `10` | Max inbound messages per sender per minute |
| `api_calls_per_minute` | integer > 0 | `60` | Reserved for future API rate limiting |

### `governance.loop_detection`

| Field | Type | Default | Description |
|---|---|---|---|
| `max_turns_per_run` | integer > 0 | `50` | Hard turn limit per agent run |
| `duplicate_call_threshold` | integer > 0 | `5` | Consecutive identical tool calls before blocking |
| `max_queue_depth_per_thread` | integer > 0 | `100` | Max queued messages per thread before rejecting |
