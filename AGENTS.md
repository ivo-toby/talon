...
- **Scheduled runs are one-shot** — scheduled queue items do not use thread provider affinity, do not restore or persist provider session IDs, do not inject schedule-thread transcript via ContextAssembler, do not run context rotation / observational memory, and do not persist skipped final assistant wrap-up text as outbound messages. Agents must call `channel_send` explicitly to notify users; successful `channel_send` deliveries are stored on the live recipient thread so replies have context.
