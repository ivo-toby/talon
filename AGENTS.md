...
- **Subscription CLI sub-agents are isolated Claude Code one-shot model adapters** — opt-in `subagentCli.claudeCode` runs bounded sub-agent generations through the daemon user's existing CLI login, without registering an `AgentProvider`. It never exposes Talon host tools, MCP servers, persona tools, or resumable sessions; Claude disables tools and persists no session. The child gets only an explicit environment allowlist: normal Claude login paths and the official `CLAUDE_CODE_OAUTH_TOKEN` headless subscription token are retained, while API-key and alternate API-routing variables are removed. Subscription cost is not fabricated as an API-dollar estimate. Do not add Codex CLI here without tool-free mode or external containment that enforces filesystem-read and network boundaries.
