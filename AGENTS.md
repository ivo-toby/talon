...
- **Subscription sub-agents are one-shot model adapters** — opt-in `subagentCli.claudeCode` runs bounded Claude Code generations through the daemon user's existing CLI login, without registering an `AgentProvider`. `subagentSandbox.codex` instead calls the separate `codex-runner` container; Codex never runs in the daemon, receives no Talon host tools/MCP/persona data/host mounts, starts a fresh ephemeral App Server turn with a sterile home/workspace, rejects non-ChatGPT auth, and fails closed on built-in tool activity. The runner's dedicated Codex account state remains sensitive and is never exposed to Talon. Subscription cost is not fabricated as an API-dollar estimate. Do not add direct Codex CLI here without external containment that enforces filesystem-read and network boundaries.
